...
bool insert(struct trienode *root,
            char *word); // a boolen function to check if everything is inserted
                         // correctly
void suggest(struct trienode *root,
             char *word); // function for suggestion of words
bool check(struct trienode *root,
           char *word);     // function to check if a word exists in dictionary
...
int main() {
  ...
  do {
    ...
    printf("\n Do you want to go back to the menu? (Press y/n)");
    scanf(" %c", &flag);
  } while (flag == 'y');
